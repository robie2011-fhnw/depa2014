...
public class StdSelectionTool implements DrawTool {
	...
	/**
	 * indicates whether the tool is in selection mode, ie. whether a selection
	 * is in progress.
	 */
	 ...
	 * 
	 * @param aView
	 *            a view this new tool shall operate on.
	 * @param aContext
	 *            a context in which this selection tool operates.
	 */
	...
	public void mouseDown(int constrainedX, int constrainedY, MouseEvent e) {
		...
		if (!isNearSelected(originalX, originalY)) {
			...
			// 4. if dragging mouse for spanning a selection, remember starting
			// position (sx0, sy0) and initialize
	...
	/**
	 * Makes Rectangles with negative width and/or height non-empty. According
	 * to the API specification, a Rectangle is empty if it has a width or a
	 * height that is smaller or equals 0. This method adjusts the Dimension of
	 * a Rectangle such that it becomes non-empty
	 * 
	 * @param r
	 *            a potentially empty Rectangle
	 * @return a non-empty Rectangle with the same Dimension as the input
	 *         parameter. <b>Note:</b>The returned reference refers to the same
	 *         Rectangle as the input parameter.
	 */
	 ...
	 * 
	 * @param outer
	 *            rectangle supposed to fully contain the inner rectangle.
	 * @param inner
	 *            rectangle supposed to be fully surrounded by outer rectangle
	...
	public void mouseDrag(int i, int j, java.awt.event.MouseEvent e) {
		...
		if (selMode) {
			...
			Rectangle selRectangle = new Rectangle(min(sx0, sx1),
					min(sy0, sy1), abs(sx1 - sx0), abs(sy1 - sy0));
		...
		for (Figure f : view.getSelection()) {
			f.move(k, l);
			view.getModel().getDrawCommandHandler()
					.addCommand(new MoveCommand(f, k, l));
		}
